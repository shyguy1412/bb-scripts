import { getPackage, getPackageInfo, PackageInfo } from './ApiWrapper';
...
export const Commands: {
    [key in ValidCommands]: (ns: NS, ...input: ND.ScriptArg[]) => Promise<void>;
} = {
    i(ns, ...input) {
    ...
    async install(ns, ...input) {
        ...
        if (typeof requestedPackage != 'string') {
            ns.tprint(
                `Error: expected package name to be string but was ${typeof requestedPackage}`,
            );
            return;
        }
        ...
        const packageInfo = await getPackageInfo(packageName, packageVersion).catch(
            ({ error }) => {
                ns.tprint(error);
            },
        );
...
export async function installPackage(
    ...
    if (visited.includes(packageInfo.name)) {
        return;
    }
