...
function resolveModule(path: string): string | null {
    ...
    const { exports, main, browser } = JSON.parse(
        load(`${modulePath}/package.json`),
    ) as PackageJson;

    if (!exports) {
        return relativePathToAbsolute(`./${browser ?? main}`, modulePath);
    }

    if (typeof exports == 'string') {
        return relativePathToAbsolute(`./${exports}`, modulePath);
    }
...
function load(path: string): string {
    const contentFile = GetServer('home').getContentFile(
        path.replace(/\.[cm]js$/, '.js'),
    );
...
export function compile(content: string): any {
    ...
    const moduleAsFunction = new Function(
        'require',
        'module',
        'exports',
...
function matchExportPattern(
    path: string,
    patterns: NonNullable<PackageJson['imports']>,
): string | null {
...
function relativePathToAbsolute(path: string, cwd: string): string {
    const fragments = `${path.startsWith('./') ? cwd : ''}/${path}`.split('/').filter(
        (f) => f != '' && f != '.',
    );
