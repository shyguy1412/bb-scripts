        ...
        >
            <div
                style={{
                    ...
                    height: '100%',
                    maxWidth: `${
                        (newCharCount.width / areas.length) * emSize.width * areas.length
                    }px`,
                    maxHeight: `${
                        (newCharCount.height / areas[0].length) * emSize.height *
                        areas[0].length
                    }px`,
                    display: 'grid',
                    ...
                    gridTemplateColumns: Array(areas[0].length).fill('minmax(0, 1fr)')
                        .join(' '),
                    background: 'red',
                    color: 'black',
                }}
                        ...
                        >
                            <CharCountContext.Provider
                                value={{
                                    width: Math.floor(
                                        newCharCount.width *
                                            ((slot.end.x - slot.pos.x) + 1) /
                                            areas[0].length,
                                    ),
                                    height: Math.floor(
                                        newCharCount.height *
                                            ((slot.end.y - slot.pos.y) + 1) /
                                            areas.length,
                                    ),
                                }}
                            >
