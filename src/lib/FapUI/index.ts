...
type ReactEventType = keyof Omit<
    React.DOMAttributes<any>,
    'children' | 'dangerouslySetInnerHTML'
>;
type ReactEvent<K extends ReactEventType, T> = Parameters<
    NonNullable<React.DOMAttributes<T>[K]>
>[0];

export type FapEvents<T> = {
    [key in ReactEventType]: (
        cb: (el: FapElement<T>, ev: ReactEvent<key, T>) => void,
    ) => FapElement<T>;
};

type AttributeSetter<T> = (attr: string | number | boolean) => FapElement<T>;
export type Attributes<T> = Omit<
    HTMLAttributes<T> & AriaAttributes<T> & DataAttributes<T>,
    keyof FapModifier<T>
>;

type HTMLAttributeKeys<T> = Capitalize<
    keyof Omit<
        React.AllHTMLAttributes<T>,
        keyof FapEvents<T> | keyof React.AriaAttributes | 'className'
    > | 'class'
>;
...
type AriaAttributeKeys = keyof React.AriaAttributes extends `aria-${infer A}` ?
    `Aria${Capitalize<A>}` :
    never;
...
type FapOverloadMap<T> = {
    [key in keyof FapElement<T>]?: FapElement<T>[key] extends
        ((...args: infer P) => FapElement<T>) ? (state: FapState<T>, ...args: P) => void :
        never;
};
...
export type FapElement<T> =
    & React.JSX.Element
    & FapEvents<T>
    & FapModifier<T>
    & Attributes<T>;
export type FapComponent<T = any> =
    & ((content?: FapContent) => FapElement<T>)
    & FapOverload<T>;

export type FapComponents =
    & {
        [key in Capitalize<keyof HTMLElementTagNameMap>]: FapComponent<
            HTMLElementTagNameMap[Uncapitalize<key>]
        >;
    }
    & {
...
const GenericFapComponent = <T extends FapElement<any>>(
    tag: string,
    overload: FapOverloadMap<T>,
    content?: FapContent,
) => {
        ...
        React.createElement(Component),
        {
            get(target, p: keyof FapElement<any>, receiver) {
                ...
                if (isEventHandler(p)) {
                    return (
                        cb: any,
                    ) => (state['events'][p] = cb.bind(undefined, receiver), receiver);
                }
                ...
                if (/^[A-Z]/.test(p)) {
                    const attribute = p.toLocaleLowerCase() as Uncapitalize<
                        keyof Attributes<any>
                    >;
                    return (
                        value: any,
                    ) => (state['attributes'][attribute] = value, receiver);
                }
                ...
                React.createElement(
                    'span',
                    {
                        ref: (el: HTMLSpanElement) =>
                            el?.parentElement?.replaceChild(child, el),
                    },
                );
        }

        const FapComponent = GenericFapComponent.bind(undefined, p, {}) as FapComponent<
            any
        >;
        FapComponent.Overload = (map) =>
            GenericFapComponent.bind(undefined, p, map) as FapComponent<any>;
        return FapComponent;
    },
});
...
export function createBinding<
    T extends (...args: any[]) => React.ReactNode,
    A extends Parameters<T>,
>(component: T, ...args: A): [React.JSX.Element, ...Bind<A>] {
...
export type WrappedMethods<
    ...
    M extends RawMethods | FapElement<any>,
> = {
...
export type WrappedComponent<
    C extends () => React.ReactNode,
    M extends RawMethods | FapElement<any>,
> = C & WrappedMethods<C, M>;

export function FapWrap<
    C extends () => React.ReactNode,
    M extends RawMethods | FapElement<any>,
>(component: C, methods: M) {
    ...
    const proxy = new Proxy(component.bind(undefined), {
        get: (t, p) => getMethod(p) ?? Reflect.get(t, p),
    }) as C & WrappedMethods<C, M>;
    return proxy;
}
