...
export class Command extends NodeEventEmitter {
    ...
    constructor(name?: string) {
        ...
        const getTerminalWidth = () => {
            const fontSizeReference =
                (0, eval)('document').querySelector('#terminal')!.firstElementChild!
                    .firstElementChild;
            if (!fontSizeReference) {
                return 0;
            }
        ...
        const write = (str: string) => {
            if (!this.ns) {
                throw new Error('Can only write if NS was passed to parse');
            }
...
- specify the name in Command constructor or using .name()`);
        ...
        if (opts.noHelp || opts.hidden) {
            cmd._hidden = true; // modifying passed command due to existing implementation
        }
    ...
    action(fn: (ns: NS, ...args: any[]) => void): Command {
        const listener = (args: any[]) => {
            ...
            return fn.apply(this, [
                new Proxy({}, { get: (t, p) => Reflect.get(this.ns!, p) }) as NS,
                ...actionArgs,
            ]);
        };
...
-  already used by option '${matchingOption.flags}'`,
    ...
    autocomplete(data: ND.AutocompleteData, rawArgs: string[]): string[] {
        ...
        for (let i = 0; i < rawArgs.length; i++) {
            ...
            if (i == rawArgs.length - 1 && !this.options.some((o) => o.is(arg))) {
                ...
                const options = this.options.filter((o) => !rawArgs.some((a) => o.is(a)))
                    .map((o) => [o.long, o.short]).flat().filter((o) => o) as string[];
                return [...options];
            }
        ...
        for (let i = 0; i < argsWithoutFlags.length; i++) {
            ...
            if (
                this.suggestions[curArg._name]?.some((s) => s.startsWith(arg) && s != arg)
            ) {
                continue;
            }
            if (!curArg.variadic) {
                curArg = argumentList.shift();
            }
        }

        if (curArg) {
            return this.suggestions[curArg._name] ?? [];
        }
    ...
    _parseCommand(operands: any, unknown: string[]) {
        if (!this.ns) {
            throw new Error('Cannot parse command without NS');
        }
        ...
        if (this._showSuggestionAfterError) {
            ...
            this.createHelp()
                ...
                .forEach((command) => {
                    ...
                    // just visible alias
                    if (command.alias()) {
                        candidateNames.push(command.alias() as string);
                    }
                });
