...
class NodeEventEmitter {
  ...
  listenerCount<K>(eventName: string | symbol): number {
    return this.events[eventName].length;
  }
}

export class Command extends NodeEventEmitter {
  commands: Command[];
  options: Option[];
  parent: null | Command;
  ...
  _actionHandler: null | ((arg: any[]) => any);
  _executableHandler: boolean;
  _executableFile: null;
  _executableDir: null;
  _defaultCommandName: null | string;
  _exitCallback: null | Function;
  _aliases: string[];
  ...
  _outputConfiguration: { writeOut: (str: any) => boolean; writeErr: (str: any) => boolean; getOutHelpWidth: () => number | undefined; getErrHelpWidth: () => number | undefined; outputError: (str: any, write: any) => any; };
  _hidden: boolean;
  _helpOption?: null | Option;
  _addImplicitHelpCommand?: boolean;
  _helpCommand?: Command;
  _helpConfiguration: {};
  runningCommand: any;
  _versionOptionName: any;
  _version: any;
  _usage: any;
  ns?: NS;
  ...
  command(nameAndArgs: string, actionOptsOrExecDesc: (object | string), execOpts: object): Command {
    let desc: (object | string) | null = actionOptsOrExecDesc;
    let opts: any = execOpts;
  ...
  configureHelp(configuration?: object): (Command | object) {
  ...
  configureOutput(configuration: object): (Command | object) {
  ...
  showHelpAfterError(displayHelp: (boolean | string) = true): Command {
    ...
    this._showHelpAfterError = displayHelp as boolean;
    return this;
  }
...
- specify the name in Command constructor or using .name()`);
   ...
   * @return {Command} `this` command for chaining
   */
  argument(name: string, description?: string, fn?: (Function | any), defaultValue?: any): Command {
  ...
  helpCommand(enableOrNameAndArgs?: string | boolean, description?: string): Command {
   ...
   * @return {Command} `this` command for chaining
   */
  addHelpCommand(helpCommand: (Command | string | boolean), deprecatedDescription: string): Command {
...
Expecting one of '${allowedValues.join("', '")}'`);
  ...
  exitOverride(fn: Function): Command {
    if (fn) {
  ...
  _callParseArg(target: (Option | Argument), value: string, previous: any, invalidArgumentMessage: string) {
    try {
...
-  already used by option '${matchingOption.flags}'`);
  ...
  _registerCommand(command: Command) {
    const knownBy = (cmd: Command) => {
      return [cmd.name()].concat(cmd.aliases());
    };

    const alreadyUsed = knownBy(command).find((name) =>
      this._findCommand(name as string),
    );
    if (alreadyUsed) {
      const existingCmd = knownBy(this._findCommand(alreadyUsed as string)!).join('|');
  ...
  addOption(option: Option): Command {
    ...
    const handleOptionValue = (val: any, invalidValueMessage: any, valueSource: any) => {
  ...
  option(flags: string, description?: string, parseArg?: (Function | any), defaultValue?: any): Command {
  ...
  requiredOption(flags: string, description: string, parseArg: (Function | any), defaultValue: any): Command {
  ...
  _prepareUserArgs(argv: any[], parseOptions: any) {
    ...
    let userArgs = argv.slice(0);

    // Find default name for program from arguments.
    if (!this._name && this._scriptPath)
      this.nameFromFilename();
  ...
  _executeSubCommand(subcommand: Command, args: any[]) {
    throw new Error('Subcommands are not yet implemented');
  }
  ...
  _processArguments() {
    const myParseArg = (argument: Argument, value: any, previous: any | any[]) => {
    ...
    const processedArgs: Argument[] = [];
    this.registeredArguments.forEach((declaredArg, index) => {
      let value: any = declaredArg.defaultValue;
      if (declaredArg.variadic) {
        // Collect together remaining arguments for passing together as an array.
        if (index < this.args.length) {
          value = this.args.slice(index);
          if (declaredArg.parseArg) {
            value = value.reduce((processed: any, v: any) => {
  ...
  _parseOptionsImplied() {
    ...
    const hasCustomOptionValue = (optionKey: string) => {
      return (
  ...
  _conflictingOption(option: Option, conflictingOption: Option) {
    ...
    const findBestOptionFromValue = (option: Option) => {
      const optionKey = option.attributeName();
      const optionValue: any = this.getOptionValue(optionKey);
    ...
    const getErrorMessage = (option: Option) => {
  ...
  unknownCommand() {
    ...
    if (this._showSuggestionAfterError) {
      ...
      this.createHelp()
        ...
        .forEach((command) => {
          candidateNames.push(command.name() as string);
          // just visible alias
          if (command.alias()) candidateNames.push(command.alias() as string);
        });
  ...
  description(str: undefined, argsDescription: undefined): string;
  description(str?: string, argsDescription?: Record<string, string>): (string | Command) {
  ...
  nameFromFilename(): Command {
    this._name = this.ns!.getScriptName();
    return this;
  }
  ...
  _getHelpContext(contextOptions: any) {
    ...
    const context: any = { error: !!contextOptions.error };
    let write;
    if (context.error) {
      write = (arg: string) => this._outputConfiguration.writeErr(arg);
    } else {
      write = (arg: string) => this._outputConfiguration.writeOut(arg);
    }
  ...
  help(contextOptions?: { error: boolean; }) {
...
Expecting one of '${allowedValues.join("', '")}'`);
  ...
  _outputHelpIfRequested(args: Array<any>) {
