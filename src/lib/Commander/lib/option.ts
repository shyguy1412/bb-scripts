...
export class Option {
  ...
  presetArg?: string;
  envVar?: string;
  parseArg?: (arg: string, previous: string[]) => string | string[];
  hidden: boolean;
  argChoices?: string[];
  ...
  argParser(fn: NonNullable<Option['parseArg']>): Option {
    this.parseArg = fn;
    return this;
  }
  ...
  makeOptionMandatory(mandatory: boolean = true): Option {
  ...
  hideHelp(hide: boolean = true): Option {
    this.hidden = !!hide;
    return this;
  }
  ...
  choices(values: string[]): Option {
    ...
    this.parseArg = (arg: string, previous: string[]) => {
      if (!this.argChoices) throw new Error('argChoices is undefined');
